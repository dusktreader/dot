...
## Problem
On Apple Silicon Macs (M1/M2/M3), Mason may install the x86_64 version of `tree-sitter-cli`, which causes
nvim-treesitter to compile parsers for the wrong architecture. This results in "no test found" errors in neotest because
the parsers fail to load.
...
The `tree-sitter-cli` has been removed from Mason's `ensure_installed` list because Mason installs the x86_64 version on
some setups, which causes architecture incompatibility issues.
