...
export default {
  ...
  args: {
    mode: {
      ...
      default: "clipboard",
      description: "'clipboard' grabs the current clipboard image; 'capture' opens an interactive region selector",
    },
  },
  async execute(args, _context) {
    try {
      const mode = args.mode ?? "clipboard"
      const result = await Bun.$`uv run ${SCRIPT} ${mode}`.text()
