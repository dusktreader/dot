...
class DotInstaller:
    ...
    def __init__(self, root: Path, override_home: Path | None = None):
        ...
        bashrc_path = self.home / ".bashrc"

        if platform.system() == "Darwin":
            bash_profile_path = self.home / ".bash_profile"
            bash_profile = bash_profile_path.read_text()
            if f"source {bashrc_path}" not in bash_profile:
                bash_profile_path.write_text(bash_profile + f"\n\nsource {bashrc_path}")
