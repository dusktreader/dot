...
- `work-dot/src/work_tools/settings.py` — schema change from `{jira_api_key, confluence_token, datadog_api_key}` to `{atlassian_pat, datadog_api_key}` with `SecretStr` types and a `field_serializer`. This is a substantive design change.
- `work-dot/.agents/instructions/work.md` — extensive content revision with accurate Jira/Atlassian documentation.
- `work-dot/tests/test_cli_creds.py` — updated to reflect the schema change (now tests `atlassian_pat`/`datadog_api_key` instead of the plan's listed fields).
