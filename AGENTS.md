# Agent instructions for dot

This repository is the `dot` dotfiles and system-bootstrap project. Read `.dot_agents/dot.md`
before doing any work here — it covers the repo layout, the `dt` CLI, and the `dt configure`
bootstrap workflow.

System-wide agent instructions live in `~/.agents/` (symlinked from `.agents/` in this repo).
Read `~/.agents/README.md` for the full directory index.


## Directory layout

| Path                    | Purpose                                               |
| ----------------------- | ----------------------------------------------------- |
| `.dot_agents/dot.md`    | Repo-specific context: layout, `dt`, `dt configure`   |
| `.agents/`              | System-wide agent files (symlinked to `~/.agents/`)   |
